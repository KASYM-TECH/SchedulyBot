...
namespace TechExtensions.SchedyBot.BLL.v2.DialogTemplates.Templates.BookingManagment.DialogSteps.Branch3
{
    public class S0_OutcomeChangeTime : IDialogStep
    {
        ...
        {
            // TODO сейчас будет очень некрасиво, но пока идей нет
            ...
            //if (booking == null)
            //    booking = (await _bookingService.GetAllByClientId(_updateContainer.Client!.Id))
            //        .FirstOrDefault(x => x.Status == BookingCompletionStatus.OnChangeTime);
            
            var currentDate = DateTime.UtcNow;
            var vacantTimes = await GetVacantTimes(currentDate, booking);
        ...
        public async Task<DialogIteration?> HandleAnswerAndGetNextIteration(string clientAnswer)
        {
            ...
            if (splittedData.Length == 2)
            {
                ...
                return new DialogIteration(TemplateId, 2, 2);
            }
        ...
        private async Task<List<InlineKeyboardButton>> GetVacantTimes(DateTime date, Booking booking)
        {
            ...
            var otherBookings = (await _bookingService.GetBookingsByExecutorIdAndDate(booking.Executor.Id, date))
